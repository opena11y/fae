...
class FileUtil {
   ...
   * @return String
   */
  public static String getContentsAllFiles(ArrayList<File> fileList, boolean commentsFlag) throws Exception {
    StringBuilder contents = new StringBuilder();

    for (File f : fileList) {
      if (debug) System.out.println(f.getAbsolutePath());
      if (commentsFlag) {
        contents.append("/* " + f.getAbsolutePath() + " */" + sep);
      }
   ...
   * @return String
   */
  public static String concatenateFiles(String inputFilename, boolean commentsFlag) throws Exception {
   ...
   * @return ArrayList of URL objects
   */

  public static ArrayList<URL> getUrlList(String inputFilename) throws Exception {
    // Check that the input file exists, etc.
    File inputFile = getNormalFile(inputFilename);
  ...
  public static void main (String args[]) {
    ...
    try {
      ...
      // test URLs
      ArrayList<URL> urls = getUrlList(urlsFile);
      for (URL url : urls) {
